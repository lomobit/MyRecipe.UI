...
export const errorsOutput = (errors: Array<string>): string =>
    `[Ошибки]:${os.EOL}\t- ${errors.join("${os.EOL}\t- ")}`;
...
export const helpSummary: string = `Примеры использования:
    ...
    2) node configurator ${needHelpParameter}${os.EOL}
${configPathParameter} (*)
    [ОБЯЗАТЕЛЬНЫЙ] Путь применяемой конфигурации приложения.${os.EOL}
${destinationPathParameter} (*)
    [ОБЯЗАТЕЛЬНЫЙ] Путь, куда будет складываться итоговая конфигурация после трансформации.${os.EOL}
${notUseParentConfigsParameter}
    Параметр, указыавющий, что к применяемой трансформации, не нужно добавлять ключи из родительских трансформаций.${os.EOL}
${needHelpParameter}
    Сводка команд.`;